...
import com.wos.services.internal.io.UnrecoverableIOException;
import com.wos.services.internal.security.BasicSecurityKey;
import com.wos.services.internal.security.ProviderCredentialThreadContext;
...
public abstract class RestStorageService {
    ...
    protected WosProperties wosProperties;
    ...
    private void addUserHeaderToRequest(Request.Builder builder) {
        ...
        for (Map.Entry<String, String> entry : userHeaderMap.entrySet()) {
            ...
            key = key.trim();
            if (!key.startsWith(this.getRestHeaderPrefix()) && !key.startsWith(Constants.WOS_HEADER_PREFIX)
    ...
    protected void initHttpClient(Dispatcher httpDispatcher) {
        ...
        OkHttpClient.Builder builder = RestUtils.initHttpClientBuilder(this, wosProperties, keyManagerFactory,
                trustManagerFactory, httpDispatcher);

        if (this.wosProperties.getBoolProperty(WosConstraint.PROXY_ISABLE, true)) {
            String proxyHostAddress = this.wosProperties.getStringProperty(WosConstraint.PROXY_HOST, null);
            int proxyPort = this.wosProperties.getIntProperty(WosConstraint.PROXY_PORT, -1);
            String proxyUser = this.wosProperties.getStringProperty(WosConstraint.PROXY_UNAME, null);
            String proxyPassword = this.wosProperties.getStringProperty(WosConstraint.PROXY_PAWD, null);
            String proxyDomain = this.wosProperties.getStringProperty(WosConstraint.PROXY_DOMAIN, null);
            String proxyWorkstation = this.wosProperties.getStringProperty(WosConstraint.PROXY_WORKSTATION, null);
        ...
        // Fix okhttp bug
        int maxConnections = this.wosProperties.getIntProperty(WosConstraint.HTTP_MAX_CONNECT,
    ...
    private void performRequestAsync(final Request request, final RequestContext context,
                                     final WosCallback<Response, ServiceException> callback) throws InterruptedException {
    ...
    private void performRequestAsync(final Request request, final RequestContext context,
                                     final WosCallback<Response, ServiceException> callback, final boolean isOEF) throws InterruptedException {
    ...
    protected void performRequestAsync(Request request, Map<String, String> requestParameters, String bucketName,
                                       boolean doSignature, WosCallback<Response, ServiceException> callback)
        ...
        context.retryMaxCount = wosProperties.getIntProperty(WosConstraint.HTTP_RETRY_MAX,
                WosConstraint.HTTP_RETRY_MAX_VALUE);
    ...
    protected Response performRequestAsync(Request request, Map<String, String> requestParameters, String bucketName,
        ...
        try {
            this.performRequestAsync(request, requestParameters, bucketName, doSignature,
                    new WosCallback<Response, ServiceException>() {
    ...
    protected Response performRequest(Request request, Map<String, String> requestParameters, String bucketName,
        ...
        try {
            ...
            int responseCode = -1;
            int retryMaxCount = wosProperties.getIntProperty(WosConstraint.HTTP_RETRY_MAX,
                    WosConstraint.HTTP_RETRY_MAX_VALUE);
            int unexpectedRetryMaxCount = wosProperties.getIntProperty(
                    ExtWosConstraint.HTTP_MAX_RETRY_ON_UNEXPECTED_END_EXCEPTION,
                    ExtWosConstraint.DEFAULT_MAX_RETRY_ON_UNEXPECTED_END_EXCEPTION);
            do {
    ...
    private boolean isProviderCredentialsInValid(ProviderCredentials providerCredentials) {
        return providerCredentials == null || providerCredentials.getWosCredentialsProvider().getSecurityKey() == null
    ...
    protected Request authorizeHttpRequest(Request request, String bucketName, String url) throws ServiceException {
        ...
        if (date != null) {
            try {
                now = ServiceUtils.getLongDateFormat().parse(date);
            ...
            date = fmt1.format(now);
        }


        builder.header(CommonHeaders.DATE, date);
    ...
    protected Request.Builder setupConnection(HttpMethodEnum method, String bucketName, String objectKey,
        ...
        if (hostname.equals(endPoint) && !isCname && bucketName.length() > 0) {
            ...
            resourceString += UrlCodecUtil.dataEncode(bucketName, "UTF-8");
        }
        if (objectKey != null) {
            //resourceString += ((pathStyle && !isCname) ? "/" : "") + RestUtils.encodeUrlString(objectKey);
            resourceString += ((pathStyle && !isCname) ? "/" : "") +  UrlCodecUtil.dataEncode(objectKey, "UTF-8");
        }
    ...
    protected boolean isKeepAlive() {
        return this.wosProperties.getBoolProperty(WosConstraint.KEEP_ALIVE, true);
    }

    protected String getEndpoint() {
        return this.wosProperties.getStringProperty(WosConstraint.END_POINT, "");
    }

    protected boolean isPathStyle() {
        return this.wosProperties.getBoolProperty(WosConstraint.DISABLE_DNS_BUCKET, false);
    }

    protected int getHttpPort() {
        return this.wosProperties.getIntProperty(WosConstraint.HTTP_PORT, WosConstraint.HTTP_PORT_VALUE);
    }

    protected int getHttpsPort() {
        return this.wosProperties.getIntProperty(WosConstraint.HTTPS_PORT, WosConstraint.HTTPS_PORT_VALUE);
    }

    protected boolean getHttpsOnly() {
        return this.wosProperties.getBoolProperty(WosConstraint.HTTPS_ONLY, true);
    }

    protected boolean isAuthTypeNegotiation() {
        return this.wosProperties.getBoolProperty(WosConstraint.AUTH_TYPE_NEGOTIATION, true);
    }

    protected boolean isCname() {
        return this.wosProperties.getBoolProperty(WosConstraint.IS_CNAME, false);
    }
