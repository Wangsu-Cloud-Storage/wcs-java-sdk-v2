...
import com.wos.services.internal.io.MayRepeatableInputStream;
...
public class RepeatableRequestEntity extends RequestBody implements Closeable {
    private static final ILogger INTERFACE_LOG = LoggerBuilder.getLogger(RepeatableRequestEntity.class);
    ...
    private static final int WRITE_BUFFER_SIZE = WosConstraint.DEFAULT_CHUNK_SIZE;

    public RepeatableRequestEntity(InputStream is, String contentType, long contentLength,
                                   WosProperties wosProperties) {
        if (is == null) {
        ...
        if (!(this.inputStream instanceof MayRepeatableInputStream)) {
            this.inputStream = new MayRepeatableInputStream(is, wosProperties
                    .getIntProperty(WosConstraint.WRITE_BUFFER_SIZE, WosConstraint.DEFAULT_WRITE_BUFFER_STREAM));
        }
